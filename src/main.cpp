...
int main() {
    ...
    const auto topology = std::make_shared<const Mesh2D>(width, height, bandwidth, latency);
    ...
    const auto collective = std::make_shared<const AllGather>(topology, chunkSize, collectivesCount);
    ...
    solverTimer.start();
    auto solver = Tacos(topology, collective);
